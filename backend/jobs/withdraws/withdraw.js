...
const _checkConfirmation = async (address, txHash, value, coin, chainId, transactionId) => {
    ...
    if (result && 'status' in result && result.status) {
        ...
        const wallet = await Wallet.findOne({ transactions: Types.ObjectId.createFromHexString(transactionId) })
        if (wallet) {
            
            const user = await User.findOne({
                wallets: wallet._id
            });
            console.log('widthdrawed', user);
...
const processWithdraw = async ({
    ...
    var result = await Transaction.findOne({ _id: Types.ObjectId.createFromHexString(transactionId) })
    if (result) {
